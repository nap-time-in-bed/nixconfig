    ...
    home-manager.users.guy = {
        ...
        home.file = {
            bashrc = {
                ...
                text = ''
                    fastfetch
                    alias ffmpegStart='mkdir -p output && for f in *.{mkv,mp4,avi,mov,ts}; do [ -f "$f" ] || continue; ffmpeg -i "$f" -c:v libx265 -preset slow -b:v 3500k -bufsize 7000k -c:a libopus -b:a 128k -c:s copy -map 0 "output/$(basename "${f%.*}").mkv"; done'
                '';
            };
        };
